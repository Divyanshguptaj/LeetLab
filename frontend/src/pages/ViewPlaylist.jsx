...
const ViewPlaylist = () => {
  const { getPlaylistsDetails, playlists, deletePlaylist, deleteProblemFromPlaylist } =
    usePlaylistStore();

  // Fetch playlists on mount (normal, will show toast)
  useEffect(() => {
    const fetchPlaylists = async () => {
      try {
        await getPlaylistsDetails(false); // show toast
  ...
  const handleDeletePlaylist = async (playlistId) => {
    try {
      ...
      await getPlaylistsDetails(true); // silent refresh
    } catch (error) {
          ...
          {playlists && playlists.length > 0 ? (
            <div className="flex flex-col gap-6">
              {playlists.map((playlist) => (
                <div
                  key={playlist.id}
                  className="collapse collapse-plus bg-base-200 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  ...
                  <div className="collapse-content bg-base-100/50 p-4 border-t border-base-300 rounded-b-xl">
                    {playlist.problems && playlist.problems.length > 0 ? (
                      <div className="flex flex-col gap-3">
                        <h4 className="text-lg font-semibold text-primary mb-2 border-b border-primary/10 pb-2">
                          Problems in this Playlist:
                        </h4>
                        {playlist.problems.map((problem) => (
                          <div
                            key={problem.id}
                            className="flex items-center justify-between bg-base-300 p-3 rounded-lg shadow-sm hover:bg-base-content/10 transition-colors duration-200"
                          >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleDeleteProblem(playlist.id, [
                                    problem.problem.id,
                                  ])
                                }
                      ...
                      <p className="text-base text-gray-500 text-center py-4 italic">
                        No problems added to this playlist yet. Start adding
                        some!
                      </p>
                    )}
                  </div>
